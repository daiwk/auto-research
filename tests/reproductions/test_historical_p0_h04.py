...
from auto_research.reproductions.historical_p0_h04 import SCORERS, diagnostics
from auto_research.reproductions.registry import get_adapter


KEYS = ("atomic-intent", "toolrec", "ssrlive", "taiji", "primal-dual-decoding", "muchator", "causal-representation", "policy-facet", "pa-bridge")
INTERNAL = ("atomic_intent", "toolrec", "ssrlive", "taiji", "primal_dual_decoding", "muchator", "causal_representation", "policy_facet", "pa_bridge")
IDS = ("2606.10357", "2606.08466", "2606.06970", "2606.03866", "2607.19357", "2605.27103", "2605.27043", "2605.16479", "2605.05855")
...
class _Sequences:
    ...
    train = tuple(tuple((user * 5 + step * 3) % 32 for step in range(18)) for user in range(10))
    validation = tuple((user * 5 + 22) % 32 for user in range(10))
    test = tuple((user * 5 + 25) % 32 for user in range(10))


class _Data:
    sequences = _Sequences()
    item_count = 32
    popularity = np.linspace(0.02, 1.0, 32)
...
def _genrec_data():
    return GenRecData(
        ...
        item_genres=tuple((("even", "popular") if index % 2 == 0 else ("odd",)) for index in range(32)),
        popularity=np.arange(32, dtype=np.float64) + 1,
    )


def test_h04_adapters_have_complete_evidence_and_executable_operators():
    papers = []
    ...
    allowed = allowed_architectures("genrec", "H04", papers)
    assert {INSTALLED_MUTATIONS[paper_id][0] for paper_id in IDS}.issubset(allowed)


def test_h04_methods_execute_nine_distinct_mechanisms():
    ...
    for key in INTERNAL:
        ...
        assert diagnostics(key, _Data, history)["finite_scores"] == 32
    assert all(not np.allclose(left, right) for i, left in enumerate(outputs) for right in outputs[i + 1 :])
    assert diagnostics("atomic_intent", _Data, history)["intent_tree_levels"] == 3
    assert diagnostics("primal_dual_decoding", _Data, history)["decoder_steps"] == 20
    assert diagnostics("causal_representation", _Data, history)["inference_overhead_parameters"] == 0


def test_h04_genrec_operators_execute():
    data = _genrec_data()
    rng = np.random.default_rng(42)
    for head in ("dynamic-sid", "causal-bottleneck", "policy-facet"):
    ...
    for context in ("atomic-intent-tree", "conversational-intent", "active-expression"):
    ...
    for reward in ("tool-calibration", "pareto-semantic-id", "primal-dual"):
        value = _reward(data, _Sequences.train[0], 7, reward)
        assert np.isfinite(value) and value > 0


def test_h04_docs_have_metadata_original_figures_and_three_seed_metrics():
    ...
    for key, paper_id in zip(KEYS, IDS):
        ...
        assert payload["manifest"]["local_code_dir"].endswith(
            f"/reproductions/{key.replace('-', '_')}"
        )
...
def test_h04_roadmap_done_only_after_artifacts_exist():
    root = Path(__file__).resolve().parents[2]
    roadmap = (root / "docs/paper-audits/2026-historical-p0-implementation-roadmap.md").read_text()
    line = next(row for row in roadmap.splitlines() if "H04 ·" in row)
