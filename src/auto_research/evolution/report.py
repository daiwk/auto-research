...
def _render_composable_report(result: EvolutionResult) -> str:
    ...
    if is_agent:
        metric_labels = (
            ...
            ("plan F1", "plan_step_f1"),
            ("invalid-tool rate", "invalid_tool_rate"),
        )
        validation_summary = "，".join(
            f"{label} `{champion.validation[key]:.4f}`"
            for label, key in metric_labels if key in champion.validation
        )
        lines += [
