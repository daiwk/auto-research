...
def allowed_architectures(model: str, direction: str, papers: list[PaperInspiration]) -> list[str]:
    ...
    if model == "genrec":
        ...
        mapped = [
            ...
            and paper.architecture.split(":", 1)[0] in {"context", "head", "reward", "distillation"}
        ]
    ...
    if model == "agent":
        operators = [
            paper.architecture
            for paper in papers
            if paper.architecture and ":" in paper.architecture
        ]
        operators = list(
            dict.fromkeys(
                [
        ...
        if operators:
            ...
            requested = [
                ...
                if operator.split(":", 1)[1].replace("_", "").replace("-", "").replace(" ", "")
                in compact_text
            ]
    ...
    if model == "micro-llm":
        ...
        for architecture, terms in priority_terms.items():
            if any(term in text for term in terms):
                ...
                values.insert(0, architecture)
        if any(
            term in text for term in ("native sparse attention", "nsa", "原生稀疏注意力")
    ...
    if model == "hyformer":
        values = ["hyformer"]
        if "longer" in requested:
            values.append("hyformer_longer")
        if "unimixer" in requested:
            values.append("hyformer_unimixer")
        if set(requested) >= {"longer", "unimixer"}:
            values.append("hyformer_longer_unimixer")
        return compatible(values)
    values = ["rankmixer_dense"]
    if "longer" in requested:
        values.append("rankmixer_longer")
    if "unimixer" in requested:
        values.append("rankmixer_unimixer")
    if set(requested) >= {"longer", "unimixer"}:
        values.append("rankmixer_longer_unimixer")
    direct_terms = {
        ...
        "rankmixer_memory_layer": ("memory layer", "训练服务一致", "in-model cache", "writeback"),
        "rankmixer_hill_index": ("hill", "hierarchical index", "分层索引", "residual quantization"),
    ...
    mapping = {
        ...
        for p in papers
        if p.architecture
        and ":" not in p.architecture
        and p.architecture not in {"longer", "unimixer"}
        and (
            not text
            or p.architecture.replace("_", "") in text.replace("-", "").replace("_", "")
            or p.title.lower().split(":", 1)[0] in text
        )
    }
...
def propose(
    ...
    architecture = (
        architectures[(index + generation - 1) % len(architectures)]
        if generation == 1
...
def _propose_llm(parent, generation, index, architectures, rng):
    ...
    if generation == 2:
        recipes = (
            ("wikitext", 0.0),
            ("mixed_narrative", 0.10),
            ("mixed_narrative", 0.20),
            ("mixed_narrative", 0.35),
            ("curriculum", 0.20),
            ("curriculum", 0.35),
        )
...
def _propose_checkpoint_vlm(parent, generation, index, architectures, rng):
    if generation == 1:
        ...
        if candidate == "pace-apc":
            return replace(
                parent,
                checkpoint_prompt_style="pace-apc",
...
def _propose_agent(parent, generation, index, operators, rng):
    ...
    if generation == 1:
        ...
        field = {
            ...
            "tool": "agent_tool_policy",
            "critic": "agent_critic",
            "policy": "agent_policy",
            "recovery": "agent_failure_recovery",
        ...
        "hypotheses": [
            {
                ...
                "architecture": t.genome.architecture,
            }
            for t in trials
        ],
        "observations": [
