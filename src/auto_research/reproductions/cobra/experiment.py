...
def reproduce_cobra(dataset_dir: Path, seed: int = 42):
    ...
    return {"paper": {"arxiv_id": "2503.02453", "title": "Sparse Meets Dense: Unified Generative Recommendations with Cascaded Sparse-Dense Representations", "url": "https://arxiv.org/abs/2503.02453", "track": "recommendation"}, "dataset": "MovieLens-100K content and chronological feedback", "setup": {"users": len(data.train), "items": data.item_count, "steps": steps, "codebooks": 2, "codebook_size": 8, "seeds": [seed, seed + 1, seed + 2]}, "training": training, "results": aggregate, "ndcg_gain_percent": 100 * (method["ndcg_at_10"] / max(base["ndcg_at_10"], 1e-12) - 1), "paper_online_ab": {"conversion_percent": 3.60, "arpu_percent": 4.15}, "scope": "Jointly trains sparse semantic-ID generation and a dense item vector conditioned on generated sparse codes; inference fuses sparse beam scores with per-code dense scores. MovieLens replaces Baidu ad logs and the production ANN/BeamFusion service."}
