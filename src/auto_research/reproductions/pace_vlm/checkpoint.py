...
def run_checkpoint(config: CheckpointConfig) -> dict[str, Any]:
    ...
    rows = load_dataset(
        config.dataset_id, revision=dataset_revision, split=config.split,
    ...
    for row in rows:
        ...
        retention, density, detail = apc_scores(
            features, global_weight=config.global_weight,
            detail_fraction=config.detail_fraction, detail_scale=config.detail_scale,
            minimum_retention=config.minimum_retention,
        )
        new_height, new_width, actual = target_resolution(
            image.height, image.width, retention=retention, patch_size=28,
        )
        resized = image.resize((new_width, new_height))
        baseline_answer, baseline_seconds, baseline_inputs = _generate(
            model, processor, image, question, torch, device, config.maximum_new_tokens,
        )
        method_answer, method_seconds, method_inputs = _generate(
            model, processor, resized, question, torch, device, config.maximum_new_tokens,
        )
        ...
        records.append({
            ...
            "baseline_seconds": baseline_seconds, "method_seconds": method_seconds,
            "baseline_input_tokens": baseline_tokens, "method_input_tokens": method_tokens,
            "retention": actual, "global_density": density, "local_detail": detail,
        })
    count = len(records)
    ...
    payload = {
        ...
        "dataset": {"name": config.dataset_id, "revision": dataset_revision, "examples": count},
        ...
        "metrics": {
            ...
            "token_reduction_percent": 100 * (1 - average("method_input_tokens") / average("baseline_input_tokens")),
        ...
        "scope": "Real Qwen2.5-VL checkpoint and public RealWorldQA subset; APC path, not paper-scale suite.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--global-weight", type=float, default=.6)
    parser.add_argument("--detail-fraction", type=float, default=.1)
    parser.add_argument("--detail-scale", type=float, default=1.5)
    parser.add_argument("--minimum-retention", type=float, default=.1)
