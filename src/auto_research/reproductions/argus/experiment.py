...
def reproduce_argus(dataset_dir: Path, seed: int = 42):
    ...
    return {"paper": {"arxiv_id": "2507.15994", "title": "Scaling Recommender Transformers to One Billion Parameters", "url": "https://arxiv.org/abs/2507.15994", "track": "recommendation"}, "dataset": "MovieLens-100K long positive sequences", "setup": {"users": len(data.train), "items": data.item_count, "steps": steps, "din_steps": FAIR_DIN_STEPS, "feedback_weight": feedback_weight, "seeds": list(seeds), "context": 32}, "training": training, "din_training": din_training, "results": aggregate, "ndcg_gain_percent": 100 * (method["ndcg_at_10"] / max(base["ndcg_at_10"], 1e-12) - 1), "ndcg_vs_din_percent": 100 * (method["ndcg_at_10"] / max(aggregate["din"]["ndcg_at_10"], 1e-12) - 1), "paper_online_ab": {"total_listening_time_percent": 2.26, "likes_percent": 6.37}, "scope": "Trains matched Transformers with the paper's autoregressive decomposition: predict feedback semantics first, project that distribution back into the state, then predict the next item. MovieLens genre feedback and a 48d model replace proprietary music signals and billion-parameter scale."}
