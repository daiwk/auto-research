(() => {
  ...
  function card(item) {
    ...
    return `<article class="ar-dashboard-card">
      ...
      ${capability ? `<div class="ar-dashboard-card__evidence ar-dashboard-card__evidence--capability"><strong>${String(item.dataset || "").includes("l2.1") ? "L2.1 隔离能力评测" : "L2 能力评测"}</strong><span>无 guide/oracle · held-out test · ${evidence.formal_comparison ? "三 seed 正式比较" : "非正式单次结果"}</span></div>` : ""}
