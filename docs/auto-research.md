...
- **论文证据层**：按方向实时搜索 arXiv；离线时使用仓库内置清单。没有本地实现的
  新论文标记为 `evidence-only`，只进入报告。
- **可执行算子层**：只有仓库里已经实现、测试并登记论文映射的结构或算法才能训练。
- **组合与调参层**：控制器可以组合兼容的已实现算子，或改变层数、维度、学习率等；
  这是新的工程假设，不是自动生成未经审计的新算法代码。

[用具体例子理解论文、算子和组合的关系 →](model-evolution.md#candidate-sources)
