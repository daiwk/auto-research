...
工业搜广推 adapter 提供三种阅读入口，避免按单一维度反复移动代码和打断复现命令：

- [按公司](by-company.md)：适合追踪各工业团队的技术路线；
- [按主题](by-topic.md)：适合横向比较召回、粗排/精排、混排、内容理解、审核风控、生成式推荐和 serving；
- [按时间](by-month.md)：适合查看技术演进和后续增量更新。

物理代码目录保持 `src/auto_research/reproductions/<adapter>/`，物理论文档保持
`<arxiv-id>-<adapter>/`；分类入口不改变物理路径。基础模型论文改从
[基础模型目录](../../foundation-models/README.md)浏览。新增工业论文需在
`PaperMetadata` 写入公司、月份、主题和量化线上 A/B 证据，且必须
`paper.has_online_ab == true`；无线上 A/B 的经典论文只有用户明确点名时才能作为具名例外。
