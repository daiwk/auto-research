...
| 字段 | 内容 |
|---|---|
| 论文链接 | [ICLR 2027](https://arxiv.org/abs/2609.03426) |
...
> 原论文模型结构与离散记忆路径。图片来自[原论文](https://arxiv.org/pdf/2609.03426)，版权归原作者所有。
<!-- paper-figure:end -->

### 核心公式

route $r$ 的离散地址为 $z_t^r=\operatorname{bits}(W_rh_t)$，阶数 $n$ 的查询键为 $(z_{t-n+1}^r,\ldots,z_t^r)$；查表 token 与零 Sink 一起进入 grouped-query attention，再以残差写回 backbone。
...
> **本地对照口径**：基线为不接 latent memory 的原 checkpoint，实验组只做 hidden-state 机制/梯度验证；当前没有同预算训练后任务指标，相对提升不适用。
