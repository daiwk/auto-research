...
| FM-003 | DONE · 本 MR | TwinKV 固定预算 KV eviction repair | 精确公式、Qwen3 真实 KV、公开 WikiText-2 长上下文、等预算质量/延迟/显存与 A100 receipt |
| MM-004 | DONE · 本 MR | PACE 视觉 token 压缩与抽取 | APC/DDAE、Qwen2.5-VL + RealWorldQA、质量/token/延迟/显存曲线与 A100 receipt |
...
| EV-005 | DONE · 本 MR | PACE 与 TwinKV checkpoint/结构算子 | 论文 ID → operator 可追踪；PACE 接入 VLM checkpoint 配方，TwinKV 接入可执行 micro-LLM attention，并与真实 checkpoint 复用 repair 函数 |
