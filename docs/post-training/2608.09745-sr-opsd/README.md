...
把自教师与 reference 做几何插值得到有效目标，再用 Rényi divergence 控制投影几何和密度比敏感度。
...
[![SR-OPSD: Self-Referenced On-Policy Self-Distillation 原论文 Figure 1](assets/paper-figure-01.png)](https://arxiv.org/html/2608.09745v1/illustration_of_method.png)

> **原论文 Figure 1（关键图）**：展示原论文方法的总体设计和关键组成。图片来自[原论文](https://arxiv.org/abs/2608.09745)，版权归原作者所有；点击图片可查看来源。
<!-- paper-figure:end -->

### 核心公式

$$
q_\beta(y)\propto p_T(y)^\beta p_{ref}(y)^{1-\beta}
$$

### 论文离线与线上效果

科学、数学和代码任务的多模型实验达到 SOTA 或具有竞争力的结果。 以上为原论文报告值；论文没有工业线上 A/B 时不作线上效果推断。
...
```bash
auto-research post-train --algorithm sr-opsd --dataset arithmetic-smoke --steps 120 --seed 42 --no-network
```
