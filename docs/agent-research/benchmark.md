...
`toolroute-l2.1`与上述 L1 mini-suite 分层：policy 接口没有 reference
answer/plan/route，也不存在提示正确 tag 的 guide；validation 只选冠军，test 只在全部
代际后运行。详细协议和三 seed 结果见
[Agent L2.1 隔离能力评测与自动进化](capability-benchmark.md)。
