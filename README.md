...
- **多模态大模型**：可从头训练 `micro-vlm`，覆盖离线 L0、Fashion-MNIST/CIFAR-10 公开图像 L1、视觉依赖对照和 connector 多轮进化；真实公开 VLM checkpoint 可生成可续跑的 ScienceQA/POPE 预测，CLIP/SigLIP 类 checkpoint 可生成 COCO/Flickr 双向检索 rank，也可用 validation 多轮进化 prompt、hint、图像预处理和解码预算，再由隔离 test 与独立 L2 scorer 评分。
...
仓库目前注册 **326 个**论文 adapter，统一事实源是
...
[视频与音频评测](docs/multimodal-models/video-audio.md)。
...
协议与固定三 seed 结果见
[Agent L2.1 隔离能力评测与自动进化](docs/agent-research/capability-benchmark.md)。
