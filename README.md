...
- **多模态大模型**：可从头训练 `micro-vlm`，覆盖离线 L0、Fashion-MNIST/CIFAR-10 公开图像 L1、视觉依赖对照和 connector 多轮进化；真实公开 VLM checkpoint 可生成可续跑的 ScienceQA/POPE 预测，也可用 validation 多轮进化 prompt、hint、图像预处理和解码预算，再由隔离 test 与独立 L2 scorer 评分。
